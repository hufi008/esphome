...
# ---------------------------------------------------------
# WAV speichern (float32 → WAV mit Header)
# ---------------------------------------------------------
def save_wav(name, wave):
    # wave: float32 stereo [-1.0 .. 1.0]
    write(f"{name}.wav", sr, wave.astype(np.float32))
    print("Generated WAV:", f"{name}.wav")

def save_pcm(name, wave):
    pcm8 = ((wave * 127) + 128).astype(np.uint8)
    pcm8.tofile(f"{name}.pcm")
    print("Generated PCM:", f"{name}.pcm")


def save_h(name, wave):
    pcm8 = ((wave * 127) + 128).astype(np.uint8)
    raw = pcm8.tobytes()
    ...
    with open(filename, "w") as f:
        f.write(f"// Auto-generated PCM header for {name}\n")
        f.write(f"std::vector<unsigned char> {array_name} = {{\n")
...
def save_b64(name, wave):
    ...
    print("Generated Base64:", f"{name}.b64")
